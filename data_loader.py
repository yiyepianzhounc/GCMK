...
from paper_work import data_pre_utils
...
class GHADataset(Dataset):
    def __init__(self, data, tokenizer, max_seq_len=128, balance=True, flag=0, movie_dic='./graph_section/电影名单.csv'):
        """
        ...
        self.movie_dic = {name: ind for name, ind in zip(self.movie_dic.name.to_list(), self.movie_dic.index.to_list())}

        self.data = data
        self.text = data[0]
        self.targets = data[1]
        self.movie_ind = [self.movie_dic.get(i) for i in data[3]]  # todo
        ...
        if balance:
            ...
            self.movie_ind = [self.movie_ind[i] for i in ind_set]
            self.user_feature = [self.user_feature[i] for i in ind_set]
            # self.user_feature = torch.tensor(self.user_feature)
            # assert not torch.any(torch.isnan(self.user_feature)), 'nan exists!'
            ...
            self.text = [self.trunate_and_pad(str(i)) for i in self.text]  # 分词之后
            self.text, self.mask, _ = zip(*self.text)

    def __getitem__(self, item):
        text = torch.tensor(self.text[item])
        mask = torch.tensor(self.mask[item])
        user_feature = torch.tensor(self.user_feature[item])
        movie_ids = torch.tensor(self.movie_ind[item])
        label = torch.tensor(self.targets[item])
        return text, mask, user_feature, movie_ids, label
...
if __name__ == '__main__':
    ...
    maoyan_Dataset_test = MaoYanDataset(root='../maoyan/data/总表.csv', page_info_path='../maoyan/data/电影主页详情.csv')
    data = maoyan_Dataset_test.get_text_user_data()
    da = GHADataset(data, tokenizer, flag=3)
    x,y,z = get_dataloader(da, batch_size=4)
    # da[1][-1] = 1
    # random_data = [list(i) for i in da]
    # random_data[1][-1] = torch.tensor(1)
    # if random_data[1][-1] == 1:
    #     print(random_data[1])
    # get_dataloader(random_data, batch_size=3)
